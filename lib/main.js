  ...
  constructor(debug) {
    ...
    this.emitter.on('request', function (message) {
      ...
      const response = new Promise(function (resolve) {
        _this.emitter.emit(`request:${ message.type }`, message.data, message);
        resolve();
      });
    ...
    this.emitter.on('response', function (message) {
      _this.emitter.emit(`job:${ message.id }`, message.data && message.data.__sb_is_error ? Communication.createError(message.data) : message.data);
    });
  }
